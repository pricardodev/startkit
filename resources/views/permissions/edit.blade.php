            ...
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <a class="text-success" href="{{ route('listar.permissao') }}">&leftarrow; Voltar para a listagem</a>

                        @if(Session::has('message'))
                                <div class="alert {{ Session::get('alert-type') }} message-alert">{{ Session::get('message') }}</div>
                            @endif

                        <form action="{{ route('atualizar.permissao', ['permissao' => $permission->id]) }}" method="post" class="mt-4" autocomplete="off">
                            @csrf
                            @method('PUT')
